...
个人授权仍需要一个飞书应用。首次安装先运行 `scripts/configure.ps1`（Windows）或 `scripts/configure.sh`（macOS/Linux），填写应用提供方给出的 App ID/Secret；然后运行 `oauth-login --full`，在浏览器登录当前使用者自己的飞书账号。完整只读权限见 [references/deployment-permissions.md](references/deployment-permissions.md)。OAuth 会申请 `offline_access`，后续命令会在令牌临近过期时自动刷新。不要共享用户 token，也不要把真实配置放进 Skill 目录。
